...
// Importación específica de botones especiales
import { RFMButtons } from "@/components/datatable/toolbar/rfm-buttons";
...
export const DataTableLayout = ({ 
  ...
  const onSearchApply = useCallback((v) => { setAppliedSearchValues(v); setPageIndex(0); }, [setAppliedSearchValues, setPageIndex]);
  const onDateChange = useCallback((r) => { setDateRange(r); setPageIndex(0); }, [setDateRange, setPageIndex]);
  const onSortChange = useCallback((by, order) => { setSortBy(by); setSortOrder(order ?? "desc"); }, [setSortBy, setSortOrder]);
      ...
      <div className="p-4">
        <div className={width ? `w-full ${widthMap[width]}` : "w-full"}>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full mt-2 pb-2 overflow-visible">
            <SearchFilter {...{ 
              searchFields, visibleFields: visibleSearchFields, setVisibleFields: setVisibleSearchFields,
              values: appliedSearchValues, onApply: onSearchApply 
            }} />

            {hasCreatedAt && <DateRangeFilter {...{ dateRange, setDateRange: onDateChange }} />}
            ...
            <div className="flex gap-2 ml-auto flex-shrink-0">
              ...
              {/* Renderizado condicional elegante */}
              {view === 'rfm' ? (
                <RFMButtons />
              ) : (
          ...
          <SmartTable {...{ 
            ...
            columnVisibility, sortBy, sortOrder, renderCell, onSortChange, actions: renderActions 
          }} />
