...
const hasCreatedAt = useMemo(
  () =>
    columns?.some(
      col => col.id === "created_at" || col.accessor === "created_at"
    ) ?? false,
  [columns]
)
        ...
        <div className={width ? `w-full ${widthMap[width]}` : "w-full"}>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full mt-2 pb-2 overflow-visible">
            <SearchFilter {...{ searchFields, visibleFields: visibleSearchFields, setVisibleFields: setVisibleSearchFields,
              values: appliedSearchValues, ssetValues: setAppliedSearchValues, onApply: v => { setAppliedSearchValues(v); setPageIndex(0) }, }} />
