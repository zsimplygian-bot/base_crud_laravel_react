...
export const SimpleForm = ({
  mode, endpoint, recordId, fields, extended_form, ExtendedForm, open, onSuccess, }: any) => {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="grid grid-cols-12 gap-x-4 gap-y-4 overflow-visible">
        {normalizedFields.map(f => {
  // Convertir width tipo "1/2" o "5/12" a col-span de Tailwind
  const colClass = (() => {
    if (!f.width) return "col-span-12"
    const [num, denom] = f.width.split("/").map(Number)
    if (!num || !denom) return "col-span-12"
    const span = Math.round((num / denom) * 12)
