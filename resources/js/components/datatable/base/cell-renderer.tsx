...
const CONFIG = {
  ...
  colores: {
    negro: "#000", negras: "#000", marrón: "#7B3F00", marron: "#7B3F00", acero: "#A8A9AD",
    cenizo: "#B2BEB5", crema: "#fff0bf", blanco: "#fff", gris: "#808080", plomo: "#808080",
    dorado: "#DAA520", rojo: "#f00", azul: "#00f", verde: "#008000", rosa: "#FFC0CB",
    naranja: "#FFA500", morado: "#800080", caramelo: "#FF7F50", beige: "#F5F5DC", fuego: "#FF4500"
  },
...
const ImageCell = memo(({ src }: { src?: string }) => {
  if (!src) return "—";
  useEffect(() => { fetch(src, { method: "HEAD" }).catch(() => {}); }, [src]); // Verifica existencia
...
const renderRaza = (v: any) => v ? <span>{v}</span> : "—";
...
const renderColor = (v: any) => {
  if (!v) return "—";
  const colors = String(v).toLowerCase().split(/\s+/).filter(w => w && !CONFIG.ignoreColorWords.has(w)).map(w => CONFIG.colores[w] ?? "#999");
...
const renderEmoji = (v: any) => <span className="text-[2.5em] leading-none">{v}</span>; // Emoji grande

const renderFecha = (v: any) => {
  const s = String(v || "");
  if (!s) return "—";
  return s.endsWith("00:00:00") ? s.replace(" 00:00:00", "") : s; // Oculta hora si es 00:00:00
};

// Mapa de render
type RenderFn = (v: any, row?: any, view?: string) => React.ReactNode;
const R: Record<string, RenderFn> = {
  fecha: renderFecha, created_at: renderFecha, updated_at: renderFecha,
  valor: v => fmt(v, "S/. "), total: v => fmt(v, "S/. "), precio: v => fmt(v, "S/. "),
  peso: v => fmt(v, "", " kg"), telefono: renderTelefono,
  ...
  raza: renderRaza, sexo: renderSexo, color: renderColor, edad: renderEdad, edad_actual: renderEdad,
};
...
export const useRenderCellContent = (defaultView?: string) => {
  return useCallback((key: string, row?: any, view?: string) => {
    ...
    if (key.toLowerCase().includes("fecha") || key.endsWith("_at")) return renderFecha(v); // Fecha genérica
...
// Registro externo
export const registerRenderer = (key: string, fn: RenderFn) => { if (typeof fn === "function") R[key] = fn; };
