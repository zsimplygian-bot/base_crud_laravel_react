...
export const DataTableFooter = ({
  ...
  const sizeItems = useMemo(
    () =>
      PAGE_SIZES.map(n => ({ label: String(n), action: () => { setPageSize(n); setPageIndex(0); }, })),
  ...
  return (
    <div className="flex flex-wrap sm:flex-nowrap items-center justify-end gap-4 mt-2 text-sm">
      ...
      <div className="flex items-center gap-2">
        Filas: <SmartDropdown {...{ items: sizeItems, triggerLabel: String(pageSize) }} />
      </div>
      <div className="flex items-center gap-1">
        {buttons.map(({ type, disabled, goToPage }, i) => {
          const Icon = ICONS[type];
          return (
            <SmartButton key={i} {...{ icon: Icon, tooltip: TOOLTIPS[type], onClick: goToPage,
                disabled, className: cn(disabled && "opacity-50 cursor-not-allowed"), }} />
          );
        })}
      </div>
